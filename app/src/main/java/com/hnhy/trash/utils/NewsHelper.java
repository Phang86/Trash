package com.hnhy.trash.utils;

import android.util.Log;

import com.google.gson.Gson;
import com.hnhy.trash.model.News;
...
public class NewsHelper {
     ...
     * @return
     */
    public static boolean saveNews(List<TrashNewsResponse.ResultBean.NewslistBean> newsList) {
        LitePal.deleteAll(News.class);
        boolean result = false;
        for (TrashNewsResponse.ResultBean.NewslistBean bean : newsList) {
            News news = new News();
     ...
     */
    public static List<TrashNewsResponse.ResultBean.NewslistBean> queryAllNews() {
        List<TrashNewsResponse.ResultBean.NewslistBean> newsList = new ArrayList<>();
        List<News> news = LitePal.findAll(News.class);
        Log.d(TAG, String.valueOf(news.size()));
        if (news != null && news.size() > 0) {
            for (int i = 0; i < news.size(); i++) {
                TrashNewsResponse.ResultBean.NewslistBean newslistBean = new TrashNewsResponse.ResultBean.NewslistBean();
